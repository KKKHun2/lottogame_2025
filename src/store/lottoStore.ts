...
type TLottoNum = number[][];

type LottoState = {
	userName: string | null;
	recommendedNumbers: number[];
	lottoData: number[][];
	lottoNumbers: TLottoNum;

	maxSavedDrawNo: number; // 마지막 저장 회차

	setUserName: (name: string | null) => void;
	setRecommendedNumbers: (nums: number[]) => void;

	addLottoNumbers: (drawNo: number, numbers: number[]) => void;
	getNextDrawNo: () => number;
};


const defaultLottoNumbers: TLottoNum = [
  [3, 6, 14, 22, 30, 41, 36],
  [1, 10, 18, 22, 28, 31, 44],
  [4, 7, 19, 26, 33, 35, 3],
  [7, 10, 19, 23, 28, 33, 18],
...
export const useLottoStore = create<LottoState>()(
	persist(
		(set, get) => ({
			userName: null,
			recommendedNumbers: [],
			...
			setRecommendedNumbers: (nums) => set({ recommendedNumbers: nums }),

			addLottoNumbers: (drawNo, numbers) => {
				// 이미 저장한 회차면 스킵
				if (drawNo <= get().maxSavedDrawNo) return;

				const next = [...get().lottoNumbers, numbers];
				set({
					lottoNumbers: next,
					lottoData: next,
					maxSavedDrawNo: drawNo
				});
			},

			getNextDrawNo: () => get().maxSavedDrawNo + 1
		}),
		{
			name: 'lottogame-storage',
			storage,
			partialize: (s) => ({
				userName: s.userName,
				recommendedNumbers: s.recommendedNumbers,
				lottoData: s.lottoData,
				lottoNumbers: s.lottoNumbers,
				maxSavedDrawNo: s.maxSavedDrawNo
			})
		}
	)
);